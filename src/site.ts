...
export const renderSite = (catalog: Catalog): ReadonlyMap<string, string> => {
  ...
  const domainOverview = rootDocuments.map(({ domain, cases: domainCases }) => {
    ...
    return `<section class="domain-overview"><header><h2>${linkDocument(domain, '')}</h2><span>${domainCases.length}ケース</span></header><p>${escapeHtml(summary)}</p>${featureItems ? `<ul>${featureItems}</ul>` : ''}</section>`;
  }).join('');
